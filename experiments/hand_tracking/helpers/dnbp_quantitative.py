...
def test_dnbp(config, epoch, mode):
	...
	with torch.no_grad():
		for v_batch, sample_batched_ in enumerate(test_dataloader):
			...
			for i_seq in range(batch_depths.shape[1]):
				...
				# Reinitialize particles throughout sequence to apply DNBP as a frame-by-frame estimator
				if mode=='estimation':
					bpn.reinit_particles(batch_depths.shape[0], batch_depths[:,0])
	...
	if mode=='estimation':
		out_file = 'evaluation_estim.pkl'
	else:
		out_file = 'evaluation_track.pkl'

	pickle.dump({"errors":np.concatenate(errors)}, open(os.path.join(epoch_folder, out_file), "wb" ) )
...
if __name__=='__main__':
	...
	test_dnbp(config=config, epoch=epoch, mode=mode)
